...
public class BrowserAgentService {
    ...
    public String navigate(String userId, String url) {
        ...
        try {
            ...
            return capturePageState(page, "navigated to " + url);
    ...
    public String screenshot(String userId) {
        ...
        try {
            ...
            return String.format("""
                    ...
                    """, file.toAbsolutePath(), title, url, base64);
        } catch (Exception e) {
    ...
    public String getQrCode(String userId) {
        ...
        try {
            ...
            if (qrInfo != null && !qrInfo.isEmpty() && !"null".equals(qrInfo)) {
                ...
                if (cw > 50 && ch > 50) {
                    byte[] bytes = page.screenshot(new Page.ScreenshotOptions()
                            ...
                            .setClip(cx, cy, cw, ch));
                    String base64 = Base64.getEncoder().encodeToString(bytes);
                    return "QR code found (" + w + "x" + h + "). Scan the image below:\n"
            ...
            // dynamically. Take a full-page screenshot so the user can scan it.
            byte[] bytes = page.screenshot(new Page.ScreenshotOptions()
                    .setType(ScreenshotType.PNG).setFullPage(false));
            String base64 = Base64.getEncoder().encodeToString(bytes);
            String title = safeTitle(page);
            String url = safeUrl(page);
            return "No specific QR code element detected. Full page screenshot:\n"
                    + "Title: " + title + "\nURL: " + url + "\n"
                    + "data:image/png;base64," + base64;
        } catch (Exception e) {
    ...
    private Page getOrCreatePage(String userId) {
        UserContext ctx = contexts.computeIfAbsent(userId, k -> {
