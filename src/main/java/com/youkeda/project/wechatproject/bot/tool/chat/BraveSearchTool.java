...
public class BraveSearchTool implements ProjectTool {
    ...
    private int maxTotalChars = 3000;
    ...
    private String formatResults(String query, List<Map<String, Object>> results) {
        ...
        sb.append("搜索 \"").append(query).append("\" 的结果（共").append(results.size()).append("条）：\n\n");
        for (int i = 0; i < results.size(); i++) {
            Map<String, Object> r = results.get(i);
            String title = str(r, "title");
            String url = str(r, "url");
            String snippet = truncate(str(r, "snippet"), maxSnippetLength);
            ...
            if (publishTime != null && !publishTime.isBlank()) {
                sb.append("   ").append(publishTime).append("\n");
            }
            sb.append("\n");

            if (sb.length() > maxTotalChars) {
                sb.append("...(后续结果已截断，请缩小搜索范围)");
                break;
            }
        }
